...
export const Route = createFileRoute('/_dashboard/fleet/instances/$instanceId/personalities/user-md')({
  component: InstancePersonalityUserMd,
})
...
function InstancePersonalityUserMd() {
  ...
  useEffect(() => {
    if (primaryAgent?.personality?.userMd) setContent(primaryAgent.personality.userMd)
  }, [primaryAgent?.personality?.userMd])
  ...
  async function handleSave() {
    ...
    try {
      await updatePersonality({
        id: primaryAgent._id,
        personality: { ...primaryAgent.personality, userMd: content },
      })
      setSaved(true)
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            User preferences{instance ? ` — ${instance.name}` : ''}
          </p>
        </div>
