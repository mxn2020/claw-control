...
function AgentChannelsBindings() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-white">Channel Bindings</h1><p className="text-sm text-slate-400 mt-1">{list.length} channel bindings</p></div>
      <Card><CardHeader><CardTitle>Bindings</CardTitle></CardHeader><CardContent>
        <div className="space-y-2">
          {list.length === 0 && <p className="text-sm text-slate-500 text-center py-6">No channel bindings configured.</p>}
          {list.map(ch => (
