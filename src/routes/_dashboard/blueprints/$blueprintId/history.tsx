...
export const Route = createFileRoute('/_dashboard/blueprints/$blueprintId/history')({ component: BlueprintHistory })
function BlueprintHistory() {
  const logs = useQuery(api.auditLogs.list, { limit: 20 })
  const entries = logs ?? []
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-white">Blueprint History</h1><p className="text-sm text-slate-400 mt-1">Change history</p></div>
      <Card><CardHeader><CardTitle>History ({entries.length})</CardTitle></CardHeader><CardContent>
        <div className="space-y-2">
          {entries.length === 0 && <p className="text-sm text-slate-500 text-center py-6">No changes recorded.</p>}
          {entries.map(e => (
